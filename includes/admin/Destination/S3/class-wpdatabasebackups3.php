...
class WPDatabaseBackupS3 {
	...
	public static function wp_db_backup_completed( &$args ) {
		...
		if ( isset( $destination_s3 ) && 1 === (int) $destination_s3 && get_option( 'wpdb_dest_amazon_s3_bucket' ) && get_option( 'wpdb_dest_amazon_s3_bucket_key' ) && get_option( 'wpdb_dest_amazon_s3_bucket_secret' ) ) {
			...
			try {
				...
				if ( get_option( 'wpdb_dest_amazon_s3_bucket' ) ) {
					if ( true === in_array( get_option( 'wpdb_dest_amazon_s3_bucket' ), $result ) ) { // phpcs:ignore
						...
						if ( $s3->putObjectFile( $args[1], $bucket_name, baseName( $args[1] ) ) ) {
